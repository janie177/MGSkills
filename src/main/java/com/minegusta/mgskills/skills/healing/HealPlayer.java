...
public class HealPlayer
{
    ...
    public static int healEntities(Player p, double amount, int radius, boolean protection, boolean speed, boolean healSelf)
    {
        ...
        new SendMessage(p, Lists.newArrayList(ChatColor.DARK_RED + "You start healing the entities around you."));
        if(healSelf)
        {
    ...
    private static void playHearts(Entity e, String name)
    {
        ...
        if(e instanceof Player) new SendMessage((Player) e, Lists.newArrayList(ChatColor.LIGHT_PURPLE + "You have been healed by " + name +"!"));
    }
    ...
    private static void healEntity(Entity e, double amount, boolean speed, boolean protection, Player p)
    {
        if(e instanceof LivingEntity)
        {
            ...
            {
                if(protection) ((Player) e).addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 120, 0, false));
