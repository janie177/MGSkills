...
import com.minegusta.mgskills.skills.Skill;
...
public class SkillCommand implements CommandExecutor
{
        ...
        if(cmd.getName().equalsIgnoreCase("skills") && s instanceof Player)
        {
            ...
            if(args.length == 2)
            {
                if(args[0].equalsIgnoreCase("show"))
                {
                    ...
                    }catch (Exception ignored){sendString("That player cannot be found! They might not exist or never played.");
                    }
                    return true;
    ...
    private void sendInfoList(List<String> list, String skill)
    {
        ...
        }catch (Exception ignored){sendList(SkillInfo.help.getInfo());}

        DetailedMPlayer mPlayer = TempData.pMap.get(p.getUniqueId());
        Skill skillClass = SkillFromString.valueOf(skill.toLowerCase()).getSkill();
